...
transfer large files efficiently over the internet. One of major
downsides of this protocol is that the torrents are often only available
for a relatively limited time. This is because, after a certain
amount of time has elapsed, there are no longer any nodes in the
network that possess certain pieces of the file. This work
investigates ways of extending the lifetime of a torrent by
intellegently selecting which peers to select and which pieces to download first based on global
...
messages to the BitTorrent protocol which propagate information on the
